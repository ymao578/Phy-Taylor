...
class ExampleProblem(tfco.ConstrainedMinimizationProblem):
    ...
    def __init__(self, y, z, ly, lz, params, upper_bound):
#         self._x = x
        self._y = y
        self._z = z
        self._ly = ly
    ...
    # itself.
        ...
        return define_loss(self._y, self._z, self._ly, self._lz, self._params, self._trainable_var)
...
def define_loss(y, z, ly, lz, params, trainable_var):
...
def try_exp(params):
    '''Initilize Taylor NN'''
    x, y, z, ly, lz, weights, biases = tnn.create_DeepTaylor_net(params)
    ...
    loss = define_loss(y, z, ly, lz, params, trainable_var)
    ...
    for f in range(params['number_of_data files_for_training'] * params['num_passes_per_file']):
        ...
        if (params['number_of_data files_for_training'] > 1) or (f == 0):  # don't keep reloading data if always same; 
            ...
            data_train_x = np.loadtxt(('./trainingdata3/%s_X%d.csv' % (params['data_name'], file_num)), delimiter=',',
                                        dtype=np.float32)
            data_train_ly = np.loadtxt(('./trainingdata3/%s_Y%d.csv' % (params['data_name'], file_num)), delimiter=',',
                                        dtype=np.float32)
            data_train_lz = np.loadtxt(('./trainingdata3/%s_Z%d.csv' % (params['data_name'], file_num)), delimiter=',',
                                        dtype=np.float32)
        ...
        for step in range(params['num_steps_per_batch'] * num_batches):
            ...
            feed_dict_train = {x: np.transpose(batch_data_train_x), ly: np.transpose(batch_data_train_ly), 
                                                                    lz: np.transpose(batch_data_train_lz)}
