...
##Construct TaylorNN ######################################################################################################################
def taylor_nn(prev_layer, weights, unkweights, biases, com_type1, com_type2, act_type, num_of_layers, expansion_order, name ='U'):
    ...
    for i in np.arange(num_of_layers):
        ...
        if name == 'U':
            ...
            pre_weight_B = tf.multiply(weights['W%s%d' % (name,i + 1)], unkweights)
            prev_layer = tf.matmul(pre_weight_B, input_epd) + biases['b%s%d' % (name,i + 1)]
            #prev_layer = tf.matmul(pre_weight_B, input_epd)
...
def initilization(widths, comT1, comT2, act, epd, dist_weights, dist_biases, phyweightsB, scale, name='U'):
    ...
    return weights, com_type1, com_type2, biases, act_type, expansion_order, unkweights
###########################################################################################################################################
...
def create_DeepTaylor_net(params):
    ...
    weights, com_type1, com_type2, biases, UN_act_type, UN_expansion_order, unkweights = initilization(widths=UN_widths,                                                                                                              comT1=params['uncheckable_com_type1'],
                                                                                           ...
                                                                                           phyweightsB=params['uncheckable_phyweightsB'],
                                                                                           scale = 0.1, name='U')
  
    y = taylor_nn(prev_layer=x, weights=weights, unkweights=unkweights, biases=biases, com_type1=com_type1, com_type2=com_type2,                              act_type=UN_act_type, num_of_layers=params['uncheckable_num_of_layers'], expansion_order=UN_expansion_order,name='U')
    
    
    C_widths = exp_length(output_size=params['checkable_output_size'], epd=params['checkable_epd'])
    C_widths = C_widths.astype(np.int64)
    C_weights, C_com_type1, C_com_type2, C_biases, C_act_type, C_expansion_order, unkweights = initilization(widths=C_widths, 
                                                                                                 comT1=params['checkable_com_type1'],
                                                                                                 comT2=params['checkable_com_type2'],
                                                                                                 act=params['checkable_act'],
                                                                                                 epd=params['checkable_epd'],   
                                                                                             dist_weights=params['checkable_dist_weights'],                                                                                              dist_biases=params['checkable_dist_biases'],
                                                                                             phyweightsB=params['uncheckable_phyweightsB'],
                                                                                             scale = 0.1, name='C')
    ...
    z = taylor_nn(prev_layer=y, weights=weights, unkweights=unkweights, biases=biases, com_type1=C_com_type1, com_type2=C_com_type2,                          act_type=C_act_type, num_of_layers=params['checkable_num_of_layers'], expansion_order=C_expansion_order,name='C')
