...
def taylor_nn(prev_layer, weights, biases, com_type1, com_type2, act_type, num_of_layers, expansion_order, name ='U'):
    ...
    for i in np.arange(num_of_layers):
        ...
        if com_type1['com1%s%d' % (name,i + 1)] == 'sigmoid':
            ...
            #one = tf.ones_like(prev_layer)
            #zero = tf.zeros_like(prev_layer)
            #label = tf.where(tf.math.less(prev_layer,0),zero, one)
            #prev_layer = prev_layer + tf.math.scalar_mul(cc, label)
            #prev_layer = tf.math.scalar_mul(-0.1, prev_layer)
        elif com_type1['com1%s%d' % (name,i + 1)] == 'lin':
            prev_layer = bb - prev_layer
            prev_layer = tf.multiply(prev_layer, aa)    
